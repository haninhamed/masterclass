	...
	Drupal.behaviors.hideCover = {
		...
		attach: function () {
			...
			$(document).ready(function() {
	    		...
	    		if ( $('.cover-img').css('background-image') != 'none' ) {
			     	// do nothing
	    		}

	    		else if ( $('.cover-img').children('.field-cover-video').length > 0 ) {
	    			// do nothing
	    		}

	    		else {
	    			
	    			$('.cover-img').addClass('is-hidden');
	    		}
     		});
		}
	};
	...
	Drupal.behaviors.scrollCard = {
		...
		attach: function () {
			...
			$(document).ready(function() {
				...
				interval = setInterval(cardRot, 5000);
			});

			function cardRot() {

				$('.no-touchevents .card.multipage .fa-chevron-right').click();
			}

			$('.no-touchevents .card.multipage').hover(function() {
				clearInterval(interval);
...
// Custom Dropdown
	...
	Drupal.behaviors.cDropD = {
		...
		attach: function () {
			...
			// 	$('.ajax-progress .throbber').addClass('fa fa-spinner');
			// });
		}
	};
